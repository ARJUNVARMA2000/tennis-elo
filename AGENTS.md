...
- **The identity tables in `config.py` have a proposer, and it is not in the pipeline.**
  ...
  candidates deterministically, asks a model through OpenRouter with web search to adjudicate
  ONLY those, throws away anything the match record refutes, and opens a PR. Nothing on the
  hourly path imports it; its OpenRouter transport uses the standard library, so no LLM client
  enters `requirements.txt`. The falsifier is the contract — read `falsify()` and its tests,
  not this summary.
