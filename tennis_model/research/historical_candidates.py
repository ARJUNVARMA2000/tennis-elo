...
from tennis_model.points.serve_return import (
    ServeReturnParams, ServeReturnState, run_serve_return, sr_params_for,
)
...
class CappedServeState(ServeReturnState):
    ...
    def _add(self, name, surf, svpt, adj_spw, rpt, adj_rpw):
        ...
        super()._add(name, surf, min(svpt, self.params.cap), adj_spw,
                     min(rpt, self.params.cap), adj_rpw)
...
def attach_capped(features, history, signal):
    ...
    require_paired(features, history.loc[main].assign(round_order=features.round_order.to_numpy()).reset_index(drop=True))
