...
INCIDENT = json.loads(
    (Path(__file__).parent / "fixtures/september22_deploy_incident.json").read_text())
ALIASES = [
    ("atp", "Zhang Zhizhen", "Zhizhen Zhang", "Z371"),
    ("atp", "Zhou Yi", "Yi Zhou", "Z0CQ"),
    ("wta", "Shi Han", "Han Shi", "329824"),
]
...
def test_pending_source_pair_broken_and_clean_producer_gate(monkeypatch, source):
    ...
    rated = [config.PLAYER_ALIASES.get(results._name_key(n), n) for n in names]
