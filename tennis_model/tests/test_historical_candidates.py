...
def frame():
    return pd.DataFrame(
        {
            "rest_diff": [60.0, -60.0, 12.0, 1.0],
            "log_days_since_diff": [5.0, -5.0, 0.5, 0.1],
            "layoff_flag_diff": [1, -1, 1, 0],
...
def test_cap_decayed_serialized_query_and_exchange():
    ...
    s._decay_to("A", t)
    s._decay_to("B", t)
    s._add("A", "Hard", 100, 0.7, 100, 0.4)
    s._add("B", "Hard", 70, 0.6, 70, 0.3)
    ...
    assert pickle.dumps(restored) == before
    with pytest.raises(ValueError):
        restored.at("2020-07-01")._add("A", "Hard", 100, 0.7, 100, 0.4)
